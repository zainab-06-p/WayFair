...
const getTrustLevelColor = (level) => {
  const map = { Platinum:'#A78BFA', Gold:'#FBBF24', Silver:'#94A3B8', Bronze:'#FB923C', Restricted:'#F87171' };
...
const glassCard = { background:'rgba(15,23,42,0.8)', backdropFilter:'blur(20px)', border:'1px solid rgba(139,92,246,0.2)', borderRadius:'14px', p:3 };
...
const SponsorshipPage = () => {
  ...
  const levelColor = trustScore ? getTrustLevelColor(trustScore.trustLevel) : '#94A3B8';
      ...
      <Container maxWidth="lg" sx={{ position:'relative', zIndex:1, pt:4, pb:6 }}>
        ...
        <motion.div initial={{ opacity:0, y:-20 }} animate={{ opacity:1, y:0 }}>
          <Box sx={{ ...glassCard, mb:3, position:'relative', overflow:'hidden' }}>
            ...
            <Typography variant="body2" sx={{ color:'#64748B', mt:0.5 }}>Build trust, sponsor new drivers, and grow your reputation</Typography>
          </Box>
        </motion.div>
            ...
            <Box sx={{ ...glassCard, mb:3, borderColor: `${levelColor}44` }}>
              <Box sx={{ display:'flex', alignItems:'center', gap:2, mb:2 }}>
                ...
                <Box sx={{ ml:'auto', textAlign:'right' }}>
                  ...
                  <Typography variant="caption" sx={{ color:'#64748B' }}>/ 1000</Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ display:'flex', gap:2, flexWrap:'wrap' }}>
                {[
                  ...
                  <Box key={s.label} sx={{ flex:'1 1 80px', background:'rgba(139,92,246,0.08)', border:'1px solid rgba(139,92,246,0.15)', borderRadius:'10px', p:1.5, textAlign:'center' }}>
                    <Typography variant="h6" sx={{ fontWeight:800, color:'#F1F5F9' }}>{s.val}</Typography>
          ...
          {TABS.map((tab,i) => (
            <Button key={tab} onClick={() => setActiveTab(tab)} variant={activeTab===tab ? 'contained' : 'outlined'}
              sx={activeTab===tab
                ...
                : { borderRadius:'12px', borderColor:'rgba(139,92,246,0.3)', color:'#94A3B8', '&:hover':{ borderColor:'#8B5CF6', color:'#F1F5F9' } }}>
              {TAB_LABELS[i]}
            </Button>
          ))}
        </Box>
        ...
        {activeTab === 'overview' && (
          <Box sx={{ display:'flex', flexDirection:'column', gap:3 }}>
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
              <Box sx={glassCard}>
                <Typography variant="h6" sx={{ fontWeight:700, color:'#F1F5F9', mb:2, fontFamily:'"Plus Jakarta Sans", sans-serif' }}>{`\u{1F3AF}`} How Sponsorship Works</Typography>
                {[
                  ...
                  <Box key={s.n} sx={{ display:'flex', gap:2, mb:2 }}>
                    ...
                    <Box>
                      <Typography sx={{ fontWeight:700, color:'#F1F5F9', fontSize:'0.95rem' }}>{s.t}</Typography>
                      <Typography variant="body2" sx={{ color:'#94A3B8' }}>{s.d}</Typography>
                    </Box>
                  </Box>
                ))}
              </Box>
            ...
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1 }}>
              <Box sx={glassCard}>
                <Typography variant="h6" sx={{ fontWeight:700, color:'#F1F5F9', mb:2, fontFamily:'"Plus Jakarta Sans", sans-serif' }}>{`\u{1F3C6}`} Trust Levels</Typography>
                <Box sx={{ display:'flex', flexDirection:'column', gap:1.5 }}>
                  {[
                    ...
                    { icon:'\u{1F948}', name:'Silver (600-799)', desc:'Can sponsor 2 drivers', color:'#94A3B8' },
                    ...
                    <Box key={l.name} sx={{ display:'flex', alignItems:'center', gap:2, p:1.5, background:'rgba(139,92,246,0.05)', border:`1px solid ${l.color}33`, borderRadius:'10px' }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" sx={{ color:'#64748B' }}>{l.desc}</Typography>
                      </Box>
                    </Box>
                  ))}
                </Box>
                ...
                <Box sx={{ ...glassCard, borderColor:'rgba(251,191,36,0.3)' }}>
                  ...
                  {pendingRequests.map(req => (
                    <Box key={req.sponsorshipID} sx={{ display:'flex', alignItems:'center', justifyContent:'space-between', p:2, background:'rgba(251,191,36,0.06)', border:'1px solid rgba(251,191,36,0.2)', borderRadius:'10px', mb:1.5 }}>
                      <Box>
                        <Typography sx={{ fontWeight:600, color:'#F1F5F9', fontSize:'0.9rem' }}>Sponsee: {req.sponseeID}</Typography>
                        <Typography variant="caption" sx={{ color:'#64748B' }}>Requested: {new Date(req.requestedAt).toLocaleDateString()}</Typography>
                      </Box>
        ...
        {activeTab === 'asSponsor' && (
          <Box sx={glassCard}>
            <Typography variant="h6" sx={{ fontWeight:700, color:'#F1F5F9', mb:3, fontFamily:'"Plus Jakarta Sans", sans-serif' }}>{`\u{1F91D}`} Drivers You're Sponsoring</Typography>
            {sponsorships.asSponsor.length === 0 ? (
              <Typography sx={{ color:'#64748B', textAlign:'center', py:4 }}>You haven't sponsored anyone yet.</Typography>
                ...
                <motion.div key={s.sponsorshipID} initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.07 }}>
                  <Box sx={{ mb:2, p:2.5, background:'rgba(139,92,246,0.06)', border:'1px solid rgba(139,92,246,0.15)', borderRadius:'12px' }}>
                    <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:1 }}>
                      <Typography sx={{ fontWeight:700, color:'#F1F5F9' }}>Sponsee: {s.sponseeID}</Typography>
                      <Chip label={s.status.toUpperCase()} size="small" sx={getStatusStyle(s.status)} />
                    </Box>
                    <Typography variant="body2" sx={{ color:'#94A3B8', mb:1 }}>Started: {new Date(s.probationStartDate).toLocaleDateString()}</Typography>
                    {s.status === 'active' && (
                      <>
                        <Typography variant="body2" sx={{ color:'#94A3B8', mb:1 }}>Ends: {new Date(s.probationEndDate).toLocaleDateString()} � Responsibility: {resp}%</Typography>
        ...
        {activeTab === 'asSponsee' && (
          <Box sx={glassCard}>
            <Typography variant="h6" sx={{ fontWeight:700, color:'#F1F5F9', mb:3, fontFamily:'"Plus Jakarta Sans", sans-serif' }}>{`\u{1F465}`} Your Sponsors</Typography>
            {sponsorships.asSponsee.length === 0 ? (
              <Typography sx={{ color:'#64748B', textAlign:'center', py:4 }}>You don't have a sponsor yet. Request one below!</Typography>
              ...
              <motion.div key={s.sponsorshipID} initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.07 }}>
                <Box sx={{ mb:2, p:2.5, background:'rgba(6,182,212,0.05)', border:'1px solid rgba(6,182,212,0.15)', borderRadius:'12px' }}>
                  <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:1 }}>
                    <Typography sx={{ fontWeight:700, color:'#F1F5F9' }}>Sponsor: {s.sponsorID}</Typography>
                    <Chip label={s.status.toUpperCase()} size="small" sx={getStatusStyle(s.status)} />
                  </Box>
                  <Typography variant="body2" sx={{ color:'#94A3B8' }}>Requested: {new Date(s.requestedAt).toLocaleDateString()}</Typography>
                  {s.acceptedAt && <Typography variant="body2" sx={{ color:'#94A3B8' }}>Accepted: {new Date(s.acceptedAt).toLocaleDateString()}</Typography>}
                  {s.status === 'active' && <Typography variant="body2" sx={{ color:'#94A3B8' }}>Probation Ends: {new Date(s.probationEndDate).toLocaleDateString()}</Typography>}
        ...
        {activeTab === 'request' && (
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
            <Box sx={glassCard}>
              <Typography variant="h6" sx={{ fontWeight:700, color:'#F1F5F9', mb:1, fontFamily:'"Plus Jakarta Sans", sans-serif' }}>{`\u2795`} Request Sponsorship</Typography>
              ...
              <Box component="form" onSubmit={handleRequestSponsorship}>
                <TextField fullWidth label="Sponsor's Pseudo ID or Wallet Address" placeholder="PSEUDO_abc123� or 0x�"
                  value={sponsorPseudoID} onChange={e => setSponsorPseudoID(e.target.value)} required sx={{ mb:2,
                    ...
                    '& .MuiInputLabel-root': { color:'#64748B' }, '& .MuiInputLabel-root.Mui-focused': { color:'#06B6D4' }, '& input': { color:'#F1F5F9' } }} />
              ...
              <Box sx={{ background:'rgba(6,182,212,0.06)', border:'1px solid rgba(6,182,212,0.2)', borderRadius:'12px', p:2 }}>
                ...
                {['Choose a sponsor who knows you personally', 'Your sponsor must have a trust score of 400+', 'Their behavior will affect their trust score for 90 days', 'Good performance helps both you and your sponsor'].map((tip,i) => (
                  <Typography key={i} variant="body2" sx={{ color:'#94A3B8', mb:0.5 }}>� {tip}</Typography>
                ))}
              </Box>
            </Box>
          </motion.div>
